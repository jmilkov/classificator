...
export default function UnmappedManagement() {
  ...
  const deleteFile = async (filename) => {
    try {
      const response = await fetch(`/api/v1/files/unmapped/${filename}`, { method: 'DELETE' });
  ...
  const openViewer = async (filename) => {
    try {
      const res = await fetch(`/api/v1/files/unmapped/read?filename=${filename}`);
  ...
  const deleteSelected = async () => {
    ...
    try {
      await Promise.all(selectedFiles.map(filename => 
        fetch(`/api/v1/files/unmapped/${filename}`, { method: 'DELETE' })
      ));
      setSelectedFiles([]);
      loadFiles();
    } catch (err) {
  ...
  const reprocessSelected = async () => {
    ...
    try {
      await Promise.all(selectedFiles.map(filename => 
        fetch(`/api/v1/files/unmapped/reprocess/${filename}`, { method: 'POST' })
      ));
      setSelectedFiles([]);
      loadFiles();
    } catch (err) {
  ...
  const loadFiles = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/v1/files/unmapped');
  ...
  return (
    <div className="app-main">
      <header className="header">
        <div>
          <h1 className="title">❓ Нераспределенные файлы (Unmapped)</h1>
          <p className="subtitle">Список файлов, требующих обработки</p>
        </div>
        <div className="header-actions" style={{ marginLeft: 'auto' }}>
          <button className="btn primary" onClick={loadFiles} disabled={loading}>Обновить список</button>
          <button className="btn primary" onClick={reprocessSelected} disabled={loading || selectedFiles.length === 0}>
              ...
              <div className="p-6">
                {viewingFile.toLowerCase().endsWith('.jpg') || viewingFile.toLowerCase().endsWith('.jpeg') || viewingFile.toLowerCase().endsWith('.png') || viewingFile.toLowerCase().endsWith('.gif') || viewingFile.toLowerCase().endsWith('.webp') ? (
                  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '400px', backgroundColor: '#f5f5f5', border: '1px solid #d1d5db', borderRadius: '4px' }}>
                    <img 
                      src={`/api/v1/files/unmapped/read?filename=${viewingFile}`} 
                      alt={viewingFile} 
